<template>
  <div class="container">
    <Form :model="groupData" ref="groupForm" :rules="ruleValidate">
      ...
      -->
      <Button type="primary" @click.once="handleSubmit('groupForm')" v-if="typeProp === 1">创建小组</Button>
      <Button type="primary" @click="handleSubmit('groupForm')" v-if="typeProp === 2">修改小组</Button>
    </Form>
  </div>
</template>
...
export default class GroupForm extends Vue {
  ...
  handleSubmit(name: string) {
    (this.$refs[name] as any).validate((valid: boolean) => {
      if (valid) {
        if (this.typeProp === 1) {
          api.createGroup({ name: this.groupData.name }).then((res: any) => {
        ...
        if (this.typeProp === 2) {
          if (this.groupData.name === this.dataProp.name) {
            ...
            api.modifyGroup({ id: this.dataProp.id, name: this.groupData.name }).then((res: any) => {
