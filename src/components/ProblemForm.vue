<template>
  <div class="problem-form">
    <Form :model="problemData" ref="problemForm">
      ...
      <FormItem>
        <h2>题目描述</h2>
        <vue-wangeditor style="text-align: left" id="editor1" :options="options" v-model="problemData.description"
      ...
      <div class="form-horizontal">
        ...
        <FormItem style="width: 50%; margin-left: 48px">
          ...
          <div style="display: flex">
            <Tag type="dot" closable color="primary" v-for="item in problemData.tagList" :key="item.name"
                 :name="item.name"
                 @on-close="deleteTag" style="margin-right: 18px">{{item.name}}
            </Tag>
            <div>
              <Input @on-enter="addTag" @on-focus="dropFocus" v-model="tag" style="width: 200px;"/>
              ...
              <div class="ivu-select-dropdown my-dropdown" v-if="myDropdown">
                <span v-for="item in allTags" :key="item.id" class="my-item" @click.stop="chooseItem(item)">
      ...
      <FormItem>
        ...
        <span>Special Judge用于答案不唯一的情况,需要自己上传判题代码。上传测试用后如需要修改, 必须重新上传对应类型的新测试用例。
          <a href="https://github.com/QingdaoU/OnlineJudge/wiki/SpecialJudge">帮助和示例</a></span>
        <code-mirror v-if="problemData.specialJudged" :logined="true"></code-mirror>
      </FormItem>
      <FormItem>
        <h2>测试数据</h2>
        <span>请将所有测试用例打包在一个zip文件中上传，所有文件要在压缩包的根目录，且输入输出文件名要以从1开始连续数字标识要对应例如：
1.in 1.out 2.in 2.out(普通题目)或者1.in 2.in 3.in(Special Judge)
      ...
      <FormItem>
        <h2>提示</h2>
        <vue-wangeditor style="text-align: left" id="editor2" :options="options" v-model="problemData.hint"
                        :init-content="dataProp.hint" :text.sync="text"></vue-wangeditor>
      </FormItem>
      <FormItem>
        <h2>来源</h2>
        <Input v-model="problemData.source" @on-enter="handleSubmit('problemForm')"/>
      </FormItem>
