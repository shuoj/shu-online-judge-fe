<template>
  <Row>
    <Col span="6" offset="9">
      <Form ref="loginForm" :model="userLogin" :rules="loginRule" class="login-form">
        <h1>用户登录</h1>
        <FormItem prop="username">
          <Input type="text" v-model="userLogin.username" placeholder="Username">
...
export default class Login extends Vue {
  ...
  handleSubmit(name: string) {
    (this.$refs[name] as HTMLFormElement).validate((valid: any) => {
      if (valid === true) {
        this.$store.dispatch('login', {      // dispatch toLogin action
