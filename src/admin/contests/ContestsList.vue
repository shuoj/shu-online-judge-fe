      ...
      <Col span="24" align="left">
        ...
        >
          <RadioGroup v-model="modalStatus">
            <Radio label="START_IN_ADVANCE">提前开始</Radio>
            <Radio label="END_IN_ADVANCE">提前结束</Radio>
          </RadioGroup>
        </Modal>
        <Row
          class="list"
        ...
        <Row v-for="(item, index) in contests" class="item" :key="item.id">
          ...
          <Col span="2">
            <div style="color: #2d8cf0" v-if="item.status === 'NOT_STARTED'">
              <a @click="reviseStatus(item.id, item.status)">未开始</a>
            </div>
            <div v-else-if="item.status === 'PROCESSING'">
              <a @click="reviseStatus(item.id, item.status)">正在进行</a>
            </div>
            <div v-else="item.status === 'ENDED'">
              <a @click="reviseStatus(item.id, item.status)">已经结束</a>
            </div>
          </Col>
          <Col span="1" align="center">
            <Button
              type="text"
              style="color: #2d8cf0;padding-top: 0"
...
import { ContestOption } from '../../types/contest'
